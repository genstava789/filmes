  ...
  return memoryCache.getOrFetch<FeaturedItem[]>(
    ...
    async () => {
      try {
        ensureContentDirExists();
        const files = getAllCustomMovieFiles();
        const diskMovies = files.map((file) => {
          try {
        ...
        // Overlay any staged drafts from MongoDB
        const stagedMovies = await getMongoMovies().catch(() => []);
        const moviesMap = new Map<string, any>();
        diskMovies.forEach((m) => moviesMap.set(m.slug, m));
        stagedMovies.forEach((m) => moviesMap.set(m.slug, m));
